...
class Topic_Model:
    ...
    def fit(self, sentences, token_lists, method=None, m_clustering=None):
        ...
        if method == 'LDA':
            ...
            # Kritika
            print("Execution in hdbcluster")
            number_clusters = floor(sqrt(len(self.vec[method])))
            print("DOC count:", len(self.vec[method]))
            print("number_clusters count:", number_clusters)
            self.cluster_model = SpectralClustering(n_clusters=number_clusters, 
                                assign_labels="discretize",
                                n_neighbors=number_clusters)
            self.cluster_model.fit(self.vec[method])
            
            # cluster_counts = pd.Series(self.cluster_model.labels_).value_counts()
            ...
            # sub_cluster_embeddings = self.vec[method][self.cluster_model.labels_ == cluster_label]

            # number_sub_clusters = floor(sqrt(sub_cluster_embeddings.shape[0]))
            # print("number_clusters count:", number_sub_clusters)

            # self.sub_cluster = SpectralClustering(n_clusters=number_sub_clusters, 
            #                      assign_labels="discretize", 
            #                      n_neighbors=number_sub_clusters)
            # print("sub_cluster_embeddings",len(sub_cluster_embeddings))
            # self.sub_cluster.fit(sub_cluster_embeddings)
